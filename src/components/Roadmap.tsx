...
interface CardSetter { scale: Setter; opacity: Setter; rot: Setter; blur: Setter; glow: Setter }
...
export default function Roadmap() {
  ...
  // Visual card update via quickSetters — never touches the track's `x`.
  const updateCards = (x: number) => {
    ...
    cardEls.current.forEach((card, i) => {
      ...
      s.scale(1 - norm * 0.12);
      s.opacity(1 - norm * 0.5);
      s.rot(norm * 10 * (cardCenter < wrapW / 2 ? -1 : 1));
      s.blur(`blur(${norm * 3.5}px)`);
      s.glow(`${norm * 0.18}`);
  ...
  useEffect(() => {
    ...
    cardSettersRef.current = cardEls.current.map((card) => {
      ...
      return {
        ...
        glow: gsap.quickSetter(card, '--glow') as Setter,
      } as CardSetter;
    });
    ...
    const drag = Draggable.create(track, {
      ...
      bounds: { minX: dimsRef.current.minX - 60, maxX: dimsRef.current.maxX + 60 },
      onPress: () => { snapTweenRef.current?.kill(); snapTweenRef.current = null; },
      onDrag: () => {
        currentXRef.current = gsap.getProperty(track, 'x') as number;
        updateCards(currentXRef.current);
      },
      ...
      onThrowComplete: () => { snapTo(nearestIdx(currentXRef.current)); },
    })[0];
    ...
    const onResize = () => {
      ...
      resizeTimer = window.setTimeout(() => {
        ...
        updateCards(t);
        if (dragRef.current) {
          dragRef.current.applyBounds({ minX: dimsRef.current.minX - 60, maxX: dimsRef.current.maxX + 60 });
        }
      }, 150);
    };
    ...
    <section id="roadmap" className="relative overflow-hidden py-24">
      ...
      <div className="relative z-10 mb-8 flex items-center justify-center gap-6">
        ...
        <div className="flex items-center gap-3">
          {PHASES.map((p, i) => (
            <button key={i} onClick={() => snapTo(i)} className="group flex items-center gap-2" aria-label={`Go to ${p.title}`}>
